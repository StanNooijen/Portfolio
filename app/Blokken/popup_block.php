...
class popup_block
{
    ...
    public function render_cms_block($popup_id, $block_name){
        ...
        $details_types = Popups_details::distinct()->pluck('type');
        $popup_type_options = '';
        $details_type_options = '';

        foreach ($popup_types as $type) {
            $selected = $data->type === $type ? 'selected' : '';
            $popup_type_options .= '<option value="' . $type . '" ' . $selected . '>' . ucfirst($type) . '</option>';
        }

        foreach ($details_types as $type) {
            $selected = $details->first()->type === $type ? 'selected' : '';
            $details_type_options .= '<option value="' . $type . '" ' . $selected . '>' . ucfirst($type) . '</option>';
        }
        ...
        if ($popup_id == '1') {
            ...
            foreach ($values as $value) {
                $buttons .= '
                    <div class="button gap-1 align-items-center" id="thingsInput">' . $value . '<i class="fa-solid fa-xmark"></i></div>
                ';
            }
            $html = '
            <div class="row display-block w-100">
                                ...
                                <div class="flex-column gap-1 w-100">
                                    ...
                                    <div class="flex-row gap-1 w-100 justify-center">
                                        <div class="flex-column rounded justify-center gap-1 bg-content p-2 w-100">
                                            <label for="popupType" class="form-label ">Popup type</label>
                                            <select id="popupType" name="popupType" class="form-control h-100">
                                               ' . $popup_type_options . '
                                            </select>
                                        </div>
                                        <div class="flex-column rounded justify-center gap-1 bg-content p-2 w-100">
                                            <label for="detailType" class="form-label ">Detail type</label>
                                            <select id="detailType" name="detailType" class="form-control h-100">
                                               ' . $details_type_options . '
                                            </select>
                                        </div>
                                    </div>
